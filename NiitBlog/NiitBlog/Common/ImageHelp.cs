...
namespace NiitBlog.Common
{
    ...
    public class ChangePicToSave
    {
        private int width;
        private int height;
        private System.Drawing.Bitmap newpic, savepic;
        private float k1, k2;

        /// <summary>
        /// 构造函数
        /// </summary>
        public ChangePicToSave()
        {
        }
        /// <summary>
        /// 保存一张小图，一张大图，和一张原图
        ...
        public void SaveThreePicture(object TheImage, string path)
        {
            string pic_larger;
            string pic_smaller;
            pic_larger = path.Substring(0, path.IndexOf("."));
            pic_larger += "_l.jpg";
            pic_smaller = path.Substring(0, path.IndexOf("."));
            pic_smaller += "_s.jpg";

            ChangePicToSave cp = new ChangePicToSave();
            cp.ChangePicToSaveMethod(TheImage, 200, 200, pic_smaller);
            cp.ChangePicToSaveMethod(TheImage, 800, 800, pic_larger);
        }
        /// <summary>
        /// 保存一张图片，并设定它的大小
        /// </summary>
        ...
        /// <param name="path">保存的文件名和路径</param>
        public void ChangePicToSaveMethod(object TheImage, int TheWidth, int TheHeight, string path)
        {
            newpic = new System.Drawing.Bitmap((System.Drawing.Image)TheImage);
            ...
            savepic = new System.Drawing.Bitmap(newpic, width, height);
            savepic.Save(@path);
        }
    }
}
