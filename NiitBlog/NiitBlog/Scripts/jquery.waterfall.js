...
;(function($, window, document){
	$.fn.waterfall = function(options){
		...
		function realign(){
			...
			// 返回顶部按钮位置
			backTop_left = $wf_col.offset().left + ($wf_col.width() + $wf_box.width()) / 2 - $backTop.width()+80; 
		...
		function showMsg(type){
			switch(type){
				...
				case 'error':
				    ...
				    $wf_result.removeClass('wf_loading').show();
					isFinish =  true;
					break;
				case 'finish':
		...
		return this.each(function(){
			...
			// 增加返回顶部按钮
			$backTop = $('<a></a>').attr('id', 'backTop').attr('title', '返回照片顶部').appendTo(document.body);
