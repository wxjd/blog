...
!function ($) {
    ...
    , show: function () {
        ...
        that.input = $('<input type="text" maxlength="30">') //后添加maxlength="30"
              ...
              if (event.keyCode == 188 || event.keyCode == 13 || event.keyCode == 9) {
                  ...
                  if (count) {
                      ...
                      if (tag.hasClass('tag-important')) {
                          if (that.options.postedit == true) {
                              that.options.del1(tag,that,count);
                          }
                          else {
                              that.remove(count - 1)
                          }
        ...
        $('<span class="tag">' + value+'</span>')
          .append($('<button type="button" class="close">&times;</button>')
            .on('click', function () {
                var $this = $(this);
                if (that.options.postedit == true) {
                    that.options.del2(value, that, $this);
                } else {
    ...
    , source: []
    , postedit: false  //编辑时ajax删除tag为true
    , del1: null //按退格键删除
    , del2: null //点击按钮删除
    }
